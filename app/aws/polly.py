#https://github.com/awsdocs/aws-doc-sdk-examples/blob/main/python/example_code/polly/polly_wrapper.py#L187
...
class PollyWrapper:
	...
	def __init__(self, aws_credentials):
		...
		self.polly_client = self.session.client("polly", region_name="us-east-1")
		self.s3_resource = self.session.resource("s3", region_name="us-east-1")
		self.voice_metadata = None

	def synthesize(
			self, text, voice, audio_format, engine='neural', lang_code=None,
...
def main():
	...
	object_polly = PollyWrapper(credentials)
	print(object_polly.get_voice_engines())
	print(object_polly.get_languages('neural'))
	print(object_polly.get_voices('neural', 'es-MX'))
	audio, vismes = object_polly.do_synthesis_task(
		"hola, Mau", 'standard', 'Mia', 'mp3', 'bucket-vplusplus', 'es-MX'
	)
	write_bytesio_to_file('/Users/abraham.lazaro/Downloads/polly.mp3', audio)
